...
func part1() int {
	...
	for _, line := range strings.Split(lines, "\n") {
		...
		val, _ := strconv.Atoi(res[0] + res[len(res)-1])
		sum += val
	}
...
func part2() int {
	...
	for _, line := range strings.Split(strings.ToLower(lines), "\n") {
		res1 := re.FindAllString(line, -1)
		first := strToNum(res1[0])
		last := first
		for i := 0; i < len(line); i++ {
			res2 := re.FindAllString(line[i:], -1)
			if len(res2) == 0 {
				break
			}
			last = strToNum(res2[len(res2)-1])
		}
...
const lines = `eighttkbtzjz6nineeight
