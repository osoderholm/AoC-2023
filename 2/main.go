...
func main() {
	fmt.Println(part1(realData))
	fmt.Println(part2(realData))
}
...
func part2(data string) int {
	...
	for _, game := range strings.Split(data, "\n") {
		_, maxKnown := splitGame(game)
		product := 1
		for _, v := range maxKnown {
			product *= v
		}
		sum += product
	}
...
func splitGame(game string) (id int, maxKnown map[string]int) {
	maxKnown = make(map[string]int)

	gameSplit := strings.Split(game, " ")
	for i := 1; i < len(gameSplit)-1; i++ {
		if i == 1 {
			id, _ = strconv.Atoi(gameSplit[i][:len(gameSplit[i])-1])
			continue
		}
		n, _ := strconv.Atoi(gameSplit[i])
		color := gameSplit[i+1]
		color = strings.TrimSuffix(strings.TrimSuffix(color, ";"), ",")
		oldN, exists := maxKnown[color]
		if !exists || oldN < n {
			maxKnown[color] = n
		}
		i++
...
const realData = `Game 1: 4 red, 8 green; 8 green, 6 red; 13 red, 8 green; 2 blue, 4 red, 4 green
