...
func part1(data string) int {
	grid, _, start := getGrid(data)

	path := make([][2]int, 0)

	path = append(path, start)
	path = append(path, grid[start[0]][start[1]][0])

	for path[len(path)-1] != start {
		nodes := grid[path[len(path)-1][0]][path[len(path)-1][1]]
		for _, node := range nodes {
			if node == path[len(path)-2] {
				continue
			}
...
func part2(data string) int {
	grid, runeGrid, start := getGrid(data)
	...
	var remove int
	for row := range runeGrid {
		var inside, onPath bool
		var pathEnter rune
		for col, r := range runeGrid[row] {
			// Check if on path
			if slices.Contains(path, [2]int{row, col}) {
				...
				if r == 'J' || r == '7' {
					if (r == 'J' && pathEnter == 'F') || (r == '7' && pathEnter == 'L') {
						inside = !inside
					}
					onPath = false
					continue
	...
	return remove - (len(path) - 1)
}

func getGrid(data string) ([][][2][2]int, [][]rune, [2]int) {
	grid := make([][][2][2]int, 0)
	...
	for i, line := range lines {
		...
		for j, r := range []rune(line) {
			...
			case 'S':
				start = [2]int{i, j}
				row = append(row, [2][2]int{{-1, -1}, {-1, -1}})
				break
			case '|':
				row = append(row, [2][2]int{
					{i + ga[North][0], j + ga[North][1]},
					{i + ga[South][0], j + ga[South][1]},
				})
				break
			case '-':
				row = append(row, [2][2]int{
					{i + ga[East][0], j + ga[East][1]},
					{i + ga[West][0], j + ga[West][1]},
				})
				break
			case 'L':
				row = append(row, [2][2]int{
	...
	startValues := make([][2]int, 0)
	startDirections := make([]int, 0)
	for _, dir := range []int{North, East, South, West} {
		other := [2]int{
			start[0] + ga[dir][0],
			start[1] + ga[dir][1],
		}
		if other[0] < 0 || other[1] < 0 || other[0] > len(grid)-1 || other[1] > len(grid[0])-1 {
			continue
		}
		var connected bool
		for _, node := range grid[other[0]][other[1]] {
			if node == start {
				connected = true
				break
			}
		}
		if len(startValues) < 2 && connected {
			startValues = append(startValues, other)
			startDirections = append(startDirections, dir)
		}
	}

	grid[start[0]][start[1]] = [2][2]int{startValues[0], startValues[1]}

	var translatedStartRune rune

	s := (startDirections[0]+1)*10 + startDirections[1] + 1

	if s == 13 || s == 31 {
		translatedStartRune = '|'
	}
	if s == 12 || s == 21 {
		translatedStartRune = 'L'
	}
	if s == 14 || s == 41 {
		translatedStartRune = 'J'
	}
	if s == 23 || s == 32 {
		translatedStartRune = 'F'
	}
	if s == 34 || s == 43 {
		translatedStartRune = '7'
	}
	if s == 24 || s == 42 {
		translatedStartRune = '-'
	}

	runeGrid[start[0]][start[1]] = translatedStartRune

	return grid, runeGrid, start
}
...
const testData5 = `FF7FSF7F7F7F7F7F---7
L|LJ||||||||||||F--J
FL-7LJLJ||||||LJL-77
F--JF--7||LJLJIF7FJ-
L---JF-JLJIIIIFJLJJ7
...
const realData = `LF-|7F-7.F--7---F7-J-F77.FLF7FF|---J-7-L-J7.F7.-.FF|7|7J7-L7.-F7|.|.F|-F-|7.---777L7-.FF77|77F7.|.FF777FJ.-L-|--7FF.F|-|--|--L.L-|F|-FJ7-JF|
