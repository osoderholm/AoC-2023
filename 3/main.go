...
func main() {
	fmt.Printf("Part 1: %d\n", part1(realData)) // 546563
	fmt.Printf("Part 2: %d\n", part2(realData)) // 91031374
}
...
func part1(data string) int {
	...
	for i := 0; i < len(lines); i++ {
		res := digitRe.FindAllStringIndex(lines[i], -1)
		if len(res) == 0 {
			continue
		}
		for _, numIdx := range res {
			begin := numIdx[0]
			if begin > 0 {
				begin -= 1
			}
			...
			// check same row
			{
				contains := symbolRe.MatchString(lines[i][begin:end])
				if contains {
					num, _ := strconv.Atoi(lines[i][numIdx[0]:numIdx[1]])
					sum += num
					continue
				}
			}
			if i > 0 {
				// Check above
				contains := symbolRe.MatchString(lines[i-1][begin:end])
				if contains {
					num, _ := strconv.Atoi(lines[i][numIdx[0]:numIdx[1]])
					sum += num
					continue
...
func part2(data string) int {
	...
	for i := 0; i < len(lines); i++ {
		// Find *
		symbols := symbolRe.FindAllStringIndex(lines[i], -1)
		if len(symbols) == 0 {
			continue
		}

		numbersCurLine := digitRe.FindAllStringIndex(lines[i], -1)
		numbersLineAbove := make([][]int, 0)
		if i > 0 {
			numbersLineAbove = digitRe.FindAllStringIndex(lines[i-1], -1)
		}
		numbersLineBelow := make([][]int, 0)
		if i < len(lines)-1 {
			numbersLineBelow = digitRe.FindAllStringIndex(lines[i+1], -1)
		}
		...
		for _, idx := range symbols {
			...
			// Check current line
			for _, numberIdx := range numbersCurLine {
				a := numberIdx[0]
				b := numberIdx[1]
				if a <= end && b > begin {
					num, _ := strconv.Atoi(lines[i][a:b])
					numbers = append(numbers, num)
				}
			}
			// Check line above
			for _, numberIdx := range numbersLineAbove {
				a := numberIdx[0]
				b := numberIdx[1]
				if a <= end && b > begin {
					num, _ := strconv.Atoi(lines[i-1][a:b])
					numbers = append(numbers, num)
				}
			}

			// Check line below
			for _, numberIdx := range numbersLineBelow {
				a := numberIdx[0]
				b := numberIdx[1]
				if a <= end && b > begin {
					num, _ := strconv.Atoi(lines[i+1][a:b])
			...
			// Assume there should only be max two
			if len(numbers) > 2 {
				panic("More than two numbers found for symbol!")
			}
...
const realData = `.....................................164.................429.35...........221....................................................34.........
